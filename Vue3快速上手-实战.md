...
采用第三种路由配置，对应的RouteLink应该采用Query检索
...
2. 原理：创建一个动态value值，再绑定一个input事件
...
- `pnpm init` create pakage.json文件

- 加入需要`axiou`包，使用命令`pnpm add axios`
...
- `.pnpm-store` 的存储位置
  ...
  - Window: `%LOCALAPPDATA%/pnpm/store`
  
  - macOS: `~/Library/pnpm/store`

也可以直接用命令去查看`.pnpm-store`的位置

```bash
pnpm store path
```

- `prune` : 从store中删除当前未被引用的包来释放store空间
...
会在根目录下生成.husk目录， there is a file named as `pre-commit`, which includs the comand lines. When we run the command `commit`, these commands will be exercuted.

add commands in the file `.husky/pre-commit`
...
### 2.2 commitline confguration

`commitlint` is used to standardize the commit information.

不能随便写commit信息，每个人按照统一标准来执行
...
页面加载的不在是图片资源，可以提高页面性能，同时，`SVG`文件比`img`要小很多
...
- 下载svg图标或者拷贝svg代码，然后自己创建按SVG

- 将`pen.svg`拷贝到`src/assets/icons`:我们刚刚在`vite.config.ts`文件里配置的。

- 加载svg代码: 格式`#icon-图标名字`

```html
<svg style="width:30px;height:30px">
  <use xlink:href="#icon-pen" fill="自定义颜色"></use>
</svg>
```
...
- 自定义插件注册svg全局组件
  ...
  - 在`main.ts`文件里导入自定义组件
  
  - ```ts
    import globalComponents from "@/component/index"
    # install self-defined components
    app.use(globalComponts)
    ```

- `app.use`函数会自动触发`component/index.ts`的`install`函数
...
```ts
<script lang='ts' setup name='AdminMenu'></script>
```

上述的组件定义方式适合外部的`import AdminMenu from xxx.vue`操作，但是组件内部并不认知该组件。如果需要内部也认识，需要传统的暴露方式定义。
...
点击按钮刷新，等同于将组件销毁，然后再重建。难点在于，什么时候`DoM`组件被销毁完成，`nextTick`函数可以实现再组件完全销毁后，再重新加载组件
...
- `document.fullscreenElement` 判断全屏状态

- `cocument.documentElement.requestFullscreen()`实现全屏
